<?php
	...
	*/
	...
	if(!hive__access($object, "admin_logging", false) AND $object["user"]->user["user_initial"] != 1) {
		...
		$trts_general 	= "creation, message, ref, section, id";
		$item_general 	= $object["lang"]->translate("be_logging")." ".$object["lang"]->translate("be_list_starthead");
		...
		$table_title   	= array(array("name" => $object["lang"]->translate("be_list_h_date")),
							    ...
							    array("name" => $object["lang"]->translate("be_list_h_ref")),
		...
		if($output1 == "deleted") {  $object["eventbox"]->warning($object["lang"]->translate("be_list_exec_idel"));  
									$object["log"]->info("[DELETION] [TABLE] ".$table_general." [ID] ".@$_POST["x_class_table_exec_deletetable"]." [UID] ".$object["user"]->user_id."", "logging_logging");}
		...
		if(is_array($value_array)) {
			foreach($value_array as $key => $value) {
				...
				$value_array[$key]["ref"] = @htmlspecialchars($value_array[$key]["ref"] ?? '');
			}
		}

<?php
	...
	*/
	...
	if(!hive__access($object, "admin_logging", false) AND $object["user"]->user["user_initial"] != 1) {
		...
		// Variables
		////////////////////////////////////////////////////////////////
		$title_general 	= $object["lang"]->translate("be_benchmark");
		$trts_general 	= "full_url, value_time, value_memory, value_load, value_queries, section, id";
		$item_general 	= $object["lang"]->translate("be_benchmark")." ".$object["lang"]->translate("be_list_starthead");
		$text_general 	= $object["lang"]->translate("be_exp_benchmark");
		$table_general 	= _TABLE_LOG_BENCHMARK_;
		$table_title   	= array(array("name" => $object["lang"]->translate("be_list_h_url")),
							    array("name" => $object["lang"]->translate("be_list_h_time")),
							    array("name" => $object["lang"]->translate("be_list_h_mem")),
							    array("name" => $object["lang"]->translate("be_list_h_cpu")),
							    array("name" => $object["lang"]->translate("be_list_h_query")),
		...
		if(@$_POST["hive_submit_flush_table"] OR @$_GET["op"] == "flush") {
			if(@$_GET["confirm"] != "confirmed") { 
				...
				if($object["csrf"]->check(@$_POST["hive_submit_flush_table_token"])) { 
					...
					$object["log"]->info("[FLUSH] [TABLE] ".$table_general." [UID] ".$object["user"]->user_id."", "logging_benchmark");
		...
		if($output1 == "deleted") {  $object["eventbox"]->warning($object["lang"]->translate("be_list_exec_idel"));  
									$object["log"]->info("[DELETION] [TABLE] ".$table_general." [ID] ".@$_POST["x_class_table_exec_deletetable"]." [UID] ".$object["user"]->user_id."", "logging_benchmark");}
